...
1. Resolve or create the workspace (`python scripts/_lib.py resolve`; if none, scaffold with
   `python scripts/init_workspace.py --workspace <dir> [--name <who>]`). This drops the profile template,
   the `dump/` folder, `WORKSPACE-MAP.md`, and the tracker, then stops.
2. Onboard the connectors: run `python scripts/setup_connectors.py` for a report of what's configured vs
