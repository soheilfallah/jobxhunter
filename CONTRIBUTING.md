...
**Read the one rule.** jobsmith's entire value is that it never invents a fact about a candidate. The master profile is the only source of truth; the skill selects, reframes, reorders, and emphasises — it never fabricates. If evidence for a job requirement isn't in the profile, that's a **gap to surface, not a blank to fill**.
...
The only exception is the `L2` "alternative-world" mode, which generates beyond the profile on purpose — and its output is never submittable.
